...
var ddaccordion={
	...
	jQuery(document).ready(function($){
		...
		$('.'+config["headerclass"]).bind("evt_accordion", function(e, isdirectclick){ //assign custom event handler that expands/ contacts a header
				...
				else if (!config["onemustopen"] || config["onemustopen"] && lastexpanded.$header && $(this).get(0)!=lastexpanded.$header.get(-1)){
		...
		$(window).bind('unload', function(){ //clean up and persist on page unload
			...
			if (config.persiststate==true && $('.'+config["headerclass"]).length>1){ //persist state?
				expandedindices=(expandedindices.length==1)? '-1c' : expandedindices //No contents expanded, indicate that with dummy '-1c' value?
