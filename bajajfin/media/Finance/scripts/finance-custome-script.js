...
$(document).ready(function () {
    $('.sidebar-nav').removeAttr('data-spy');

    //MOBILE CAL SELECT LIST LINK
    $('.selectTab').change(function () {
        var link = $(this).val();
        window.location = link;
    });

    //MOBILE CAL SELECT LIST LINK END'
    ...
    if (LinkHref.indexOf('business-loan-mobile-form') > -1) {
        window.location.href = "/finance/business-loan/business-loan-lead-form.aspx";
    }
	
	var hash = location.hash;
	if(hash.indexOf('tab') > -1){
	$('.sidebar-nav li a[href =' + hash + ']').click(); 
	}
});
...
/* tab open - click on content html */

function onlinkClick(hrefID) {
    $('.sidebar-nav li a[href=' + hrefID + ']').click();
}